...
public class EnemyMove : MonoBehaviour
{
    ...
    void Update()
    {
        if (CanPatrol == true)
        {
            ...
            if (DistanceToTarget > StopDistance) //목표 타겟과 접촉 후 조건0의 애니메이션을 실행하는 동시에  다음 타켓으로 이동
            {
            ...
            if (DistanceToTarget < StopDistance) //목표 타겟과 접촉 시 조건1의 애니메이션을 실행하는 동시에 ai기능 비활성
            {  
                Nav.isStopped = true;
    ...
    IEnumerator StartElements()
    {
        ...
        TheTarget = Target1;
        Nav.avoidancePriority = Random.Range(5, 65); //적이 플레이어를 밀지 않게 하기 위해.
        CanPatrol = true;
    }
}